...
class PaceDCMotor(DCMotor):
    """Pace DC Motor actuator model with encoder bias and tanh torque scaling.
    ...
    def __init__(self, cfg: PaceDCMotorCfg, *args, **kwargs):
        ...
        if isinstance(cfg.tanh_scale, (list, tuple)):
            ...
            self.tanh_scale = torch.tensor(cfg.tanh_scale, device=self._device).unsqueeze(0).repeat(self._num_envs, 1)
        else:
            self.tanh_scale = torch.full((self._num_envs, self.num_joints), cfg.tanh_scale, device=self._device)
    ...
    def update_tanh_scale(self, tanh_scale: torch.Tensor):
        """Update the tanh scale factor for torque processing.

        Args:
            tanh_scale: Scale factor tensor with shape (num_envs, num_joints) or broadcastable.
        """
    ...
    def compute(
        ...
        # Apply tanh scaling to torque commands: tanh(w * torque)
        control_action_sim.joint_efforts = torch.tanh(self.tanh_scale * control_action_sim.joint_efforts)
