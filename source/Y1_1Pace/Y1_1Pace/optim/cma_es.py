...
class CMAESOptimizer:
    def __init__(self, bounds, population_size, log_dir, joint_order, max_iteration, data, device, epsilon=None, sigma=0.5, save_interval=10, save_optimization_process=False):
        ...
        self.smoothing_coefficient_idx = slice(3 * num_joints, 4 * num_joints)
    ...
    def update_simulator(self, articulation, joint_ids, initial_position):
        ...
        for drive_type in articulation.actuators.keys():
            ...
            articulation.actuators[drive_type].update_smoothing_coefficient(self.sim_params[:, self.smoothing_coefficient_idx][:, drive_joint_idx])
    ...
    def _print_iteration(self):
        ...
        print("Smoothing coefficient: ", self.sim_params[min_index, self.smoothing_coefficient_idx].tolist())
    ...
    def _log(self):
        ...
        for i in range(len(self.joint_order)):
            self.writer.add_histogram("4_Smoothing coefficient/distribution_" + self.joint_order[i], self.sim_params[:, self.smoothing_coefficient_idx][:, i], self.iteration_counter)
            ...
            self.writer.add_scalar("4_Smoothing coefficient/best_" + self.joint_order[i], self.sim_params[min_score_index, self.smoothing_coefficient_idx][i].item(), self.iteration_counter)
