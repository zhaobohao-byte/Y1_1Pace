...
parser.add_argument("--constant_velocity", type=float, default=0.5, help="Constant velocity for joint motion (rad/s). If None, uses zero actions.")
...
def main():
    ...
    # disturbance parameters: apply constant disturbance at 2s for 1s duration
    disturbance_start_time = 2.0  # seconds
    disturbance_duration = 1.0  # seconds
    disturbance_end_time = disturbance_start_time + disturbance_duration
    disturbance_start_step = int(disturbance_start_time / sim_dt)
    disturbance_end_step = int(disturbance_end_time / sim_dt)
    # disturbance torque magnitude (Nm) - can be adjusted
    disturbance_torque = 10.0 # Nm
    print(f"[INFO]: Will apply {disturbance_torque} Nm disturbance from {disturbance_start_time}s to {disturbance_end_time}s")
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            # apply external disturbance if in disturbance time window
            # Add disturbance torque directly to control actions (torque control mode)
            if disturbance_start_step <= step_count < disturbance_end_step:
                # add constant disturbance torque to the joint
                # In torque control, actions are torques, so we can directly add disturbance
                disturbance_torques = torch.zeros_like(actions)
                disturbance_torques[:, joint_indices] = disturbance_torque
                actions = actions + disturbance_torques
